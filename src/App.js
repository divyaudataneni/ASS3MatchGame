...
class App extends Component {
  state = {
    newImageUrl: imagesList[0].imageUrl,
  ...
  onClickThumbnail = id => {
    ...
    const isCorrect = getImage[0].imageUrl === newImageUrl
    if (isCorrect) {
      this.setState(prevState => ({score: prevState.score + 1}))
      const newImageNo = parseInt(Math.random() * imagesList.length)
      this.setState({newImageUrl: imagesList[newImageNo].imageUrl})
    } else {
      clearInterval(this.timerId)
      this.setState({isPlaying: false})
    }
  ...
  render() {
    ...
    const newImagesList = imagesList.filter(each => each.category === category)
          ...
          {isPlaying && (
            <div>
              <div className="image-view-div">
                <img src={newImageUrl} alt="match" className="imageUrl" />
              </div>
                  ...
                  <li key={each.tabId}>
                    <button
                      ...
                      className={`tabs-heading ${
                        category === each.tabId ? 'active' : ''
                      }`}
                      onClick={() => this.onUpdateTab(each.tabId)}
                    >
                      {each.displayText}
                    </button>
                  </li>
              ...
              <ul className="images-list-cont">
                {newImagesList.map(each => (
                  <li key={each.id}>
                    <button
                      type="button"
                      className="btn-image"
                      onClick={() => this.onClickThumbnail(each.id)}
                    >
                      <img
